...
public class IFF extends MetaSentence {
    ...
    public MetaInference reason(Proof p, int inferenceNum) {
        if (args.get(0) instanceof MODELS && args.get(1) instanceof MODELS) {
            ...
            MODELS m1 = new MODELS(t1, arg1.getSentence(), true, inferenceNum),
                    m2 = new MODELS(t1, arg2.getSentence(), false, inferenceNum),
                    m3 = new MODELS(t2, arg1.getSentence(), false, inferenceNum),
                    m4 = new MODELS(t2, arg2.getSentence(), true, inferenceNum);

            MetaSentence s = new AND(new NOT(new AND(m1, m2, new HashSet<>(Collections.singletonList(t1)))),
                    new NOT(new AND(m4, m3, new HashSet<>(Collections.singletonList(t2)))),
